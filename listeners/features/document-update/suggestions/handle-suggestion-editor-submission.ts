...
export const handleSuggestionEditorSubmission = async ({
  ...
  try {
    ...
    const diffBlockIndex = blocks.findIndex(
      (block: any) =>
    ...
    if (diffBlockIndex !== -1) {
      ...
      const actionsBlockIndex = blocks.findIndex(
        (block: any) =>
          block.type === 'actions' && block.elements?.some((element: any) => element.action_id === 'edit_update'),
      );
