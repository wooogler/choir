...
import { DocumentMetadata, SearchParams } from "./types";
...
export class EnhancedSearchService {
  ...
  public analyzeMessages(messages: SlackMessage[]): MessageAnalysis {
    ...
    const mentionedFiles = Array.from(new Set(
      (combinedText.match(filePatterns) || []).map(f => f.toLowerCase())
    ));

    // 섹션/헤딩 패턴 매칭 (# 헤딩, "section", "chapter" 등)
    const sectionPatterns = /(#+ [^#\n]+|section [a-zA-Z0-9\s]+|chapter [a-zA-Z0-9\s]+)/gi;
    const mentionedSections = Array.from(new Set(
      (combinedText.match(sectionPatterns) || [])
    ...
    const entities = Array.from(new Set(
      (combinedText.match(entityPatterns) || [])
        .map(e => e.replace('@', '').toLowerCase())
    ));

    // 우선순위 계산 (더 많은 컨텍스트가 있을수록 높은 점수)
    const priority = Math.min(1.0, 
      (mentionedFiles.length * 0.4 + 
       mentionedSections.length * 0.3 + 
       entities.length * 0.1) / 2
    );
  ...
  public createSearchStrategy(analysis: MessageAnalysis): SearchStrategy {
    ...
    const preferredFileTypes = analysis.mentionedFiles.length > 0 
    ...
    // 섹션 필터링
    const preferredSections = analysis.mentionedSections.length > 0 
      ? analysis.mentionedSections 
      : undefined;

    // 노드 타입 제외 (우선순위가 매우 낮을 때만 섹션 요약 제외)
    // 빈 섹션은 업데이트가 필요하므로 가능한 포함
    const excludeNodeTypes = analysis.priority < 0.1 
      ? ['section-summary'] 
      : undefined;
  ...
  private calculateMetadataScore(
    ...
    if (analysis.mentionedFiles.length > 0) {
      ...
      const hasFileMatch = analysis.mentionedFiles.some(file => 
        fileName.includes(file) || file.includes(fileName.split('.')[0])
      );
      if (hasFileMatch) {
    ...
    if (analysis.mentionedSections.length > 0) {
      ...
      const hasSectionMatch = analysis.mentionedSections.some(section => 
        sectionName.includes(section) || 
        headingPath.some(heading => heading.includes(section))
      );
    ...
    if (analysis.entities.length > 0 && metadata.entityMentions) {
      const entityMatches = metadata.entityMentions.filter(entity => 
        analysis.entities.some(queryEntity => 
          entity.toLowerCase().includes(queryEntity) || 
          queryEntity.includes(entity.toLowerCase())
        )
      ).length;
  ...
  public async performEnhancedSearch(
    ...
    // 4. 향상된 검색 수행
    const searchResults = await this.vectorStore.enhancedSearch({
      ...strategy.baseParams,
      k: k * 2 // 더 많은 결과를 가져와서 후처리
    });
    ...
    // 5. 메타데이터 기반 재점수화
    const rescored = searchResults.map(result => ({
      document: result.document,
      score: result.score * this.calculateMetadataScore(result.document, analysis, strategy)
    }));
    ...
    const finalResults = topResults.map(result => result.document);
    
    console.info(`Enhanced search returned ${finalResults.length} results`);
    console.info("Top results:", topResults.map(result => ({
