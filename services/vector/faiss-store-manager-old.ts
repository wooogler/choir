...
export class FAISSStoreManager {
   ...
   */
  async similaritySearch(query: string, k = 4, filter?: Record<string, any>): Promise<Document<DocumentMetadata>[]> {
    try {
      if (!this.isInitialized) {
      ...
      if (processedResults.length > 0) {
        ...
        Logger.info(
          `FAISS: First result sample - File: ${firstResult.metadata?.fileName}, NodeId: ${firstResult.metadata?.nodeId}, Content: "${firstResult.pageContent.substring(0, 100)}..."`,
        );
      }
  ...
  getDocuments(): Document<DocumentMetadata>[] {
    // 멀티파일 모드인 경우 멀티파일 매니저에서 문서 수 반환
    if (this.useMultiFileMode) {
   ...
   */
  getDiagnostics(): any {
    // 멀티파일 모드인 경우 멀티파일 매니저의 진단 정보 반환
    if (this.useMultiFileMode) {
