...
import type { DocumentMetadata, EnhancedSearchResult, SearchParams } from './types';
...
export class SearchService {
  ...
  private sectionIdToDocuments: Map<string, Document<DocumentMetadata>[]> = new Map();
  private sectionSummaries: Map<string, Document<DocumentMetadata>> = new Map();
  private entityToDocuments: Map<string, Document<DocumentMetadata>[]> = new Map();

  constructor(store: MemoryVectorStore, embeddingService: EmbeddingService, logger: Console = console) {
    this.store = store;
  ...
  public async similaritySearch(query: string, k = 5): Promise<Document<DocumentMetadata>[]> {
    try {
      ...
      try {
        ...
        const searchResults = await this.store.similaritySearch(preprocessedQuery, k);
        ...
        if (results.length === 0) {
          this.logger.warn("No valid similarity results found using LangChain's method");
          // 디버그 정보 추가
  ...
  private extractImportantKeywords(query: string): string[] {
    ...
    const foundKeywords = keywords.filter((keyword) => query.toLowerCase().includes(keyword.toLowerCase()));
  ...
  private debugVectorStoreState(): void {
    try {
      ...
      this.logger.info(`Vector store debug info: memoryVectors count = ${memoryVectors?.length || 0}`);
      ...
      if (memoryVectors && memoryVectors.length > 0) {
        ...
        this.logger.info(
          `Sample vector structure: ${JSON.stringify({
            ...
            valuesLength: Array.isArray(sampleVector.values) ? sampleVector.values.length : 0,
        ...
        // 벡터의 첫 번째 값들 확인
        if (Array.isArray(sampleVector.values) && sampleVector.values.length > 0) {
          this.logger.info(`Vector first 3 values: ${sampleVector.values.slice(0, 3).join(', ')}`);
        }
      }
    } catch (error) {
   ...
   */
  public async enhancedSearch(params: SearchParams): Promise<EnhancedSearchResult[]> {
    try {
      ...
      // 기본 벡터 검색 수행
      const searchResults = await this.store.similaritySearchWithScore(params.query, k * 2);

      // 결과 후처리 및 향상
      let enhancedResults: EnhancedSearchResult[] = searchResults.map(([doc, score]) => {
        ...
        // 섹션 요약 부스팅
        if (boostSectionSummaries && metadata.nodeType === 'section-summary') {
          finalScore *= 1.2; // 섹션 요약은 20% 점수 증가
        }

        // 엔티티 매칭 부스팅
        if (boostByEntityMatch && metadata.entityMentions && queryEntities.length > 0) {
          const matchCount = metadata.entityMentions.filter((entity) => queryEntities.includes(entity)).length;
        ...
        // 노드 타입 필터링
        if (params.filterByNodeType && metadata.nodeType && !params.filterByNodeType.includes(metadata.nodeType)) {
          finalScore = 0; // 필터링된 노드 타입은 제외
        }

        // 섹션 ID 필터링
        if (params.filterBySectionId && metadata.sectionId !== params.filterBySectionId) {
   ...
   * 검색 결과에 관련 컨텍스트 추가
   */
  private async addRelatedContext(results: EnhancedSearchResult[]): Promise<EnhancedSearchResult[]> {
    for (const result of results) {
      ...
      if (metadata.nodeId && metadata.totalChunks && metadata.totalChunks > 1) {
        result.relatedChunks = this.findRelatedChunks(metadata.nodeId, metadata.chunkIndex);
      }
    }
   ...
   * 동일 노드의 관련 청크 찾기
   */
  private findRelatedChunks(nodeId: string | undefined, currentChunkIndex?: number): Document<DocumentMetadata>[] {
    ...
    for (const doc of Array.from(this.nodeIdToDocument.values())) {
      if (doc.metadata.nodeId === nodeId && doc.metadata.chunkIndex !== currentChunkIndex) {
        relatedChunks.push(doc);
      }
    }

    // 청크 인덱스 순으로 정렬
    return relatedChunks.sort((a, b) => (a.metadata.chunkIndex || 0) - (b.metadata.chunkIndex || 0));
  }
   ...
   * 특정 섹션의 문서들 가져오기
   */
  public getDocumentsBySection(sectionId: string): Document<DocumentMetadata>[] {
  ...
  public findDocumentsByEntity(entity: string): Document<DocumentMetadata>[] {
    ...
    // 부분 일치 검색
    const matchingEntities = Array.from(this.entityToDocuments.keys()).filter((key) =>
      key.toLowerCase().includes(lowercaseEntity),
    );
