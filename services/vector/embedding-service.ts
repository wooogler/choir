...
export class EmbeddingService {
  ...
  public isVectorValid(vector: number[] | null | undefined): boolean {
    ...
    const allInvalid = vector.every((val) => val === undefined || val === null || Number.isNaN(val));
    if (allInvalid) {
      return false;
    }

    // 유효한 숫자가 최소 10% 이상 있는지 확인
    const validCount = vector.filter((val) => val !== undefined && val !== null && !Number.isNaN(val)).length;
  ...
  public async createEmbeddings(texts: string[]): Promise<number[][]> {
    try {
      ...
      this.logger.info(
        `Creating embeddings for ${texts.length} texts. First text sample: "${texts[0].substring(0, 50)}${
          texts[0].length > 50 ? '...' : ''
        }"`,
      );
      ...
      try {
        ...
        this.logger.info(`Successfully created ${embeddings.length} embeddings`);
  ...
  public async createQueryEmbedding(query: string): Promise<number[]> {
    try {
      ...
      try {
        ...
        this.logger.info(`Query embedding created in ${duration}ms, dimension: ${embedding.length}`);
  ...
  public calculateSafeCosine(v1: number[], v2: number[]): number {
    try {
      ...
      if (validDimensions < minLength * 0.1) {
        // 10% 미만의 차원만 유효하면
        this.logger.warn(`Too few valid dimensions: ${validDimensions}/${minLength}`);
        return 0;
      }
