...
export class VectorCacheManager {
  ...
  public async generateContentHash(files: MarkdownFile[]): Promise<string> {
    try {
      ...
      const contentStrings = files.map((file) => `${file.path}:${file.content}`);
  ...
  public async saveEmbeddingsCache(data: EmbeddingCacheData): Promise<boolean> {
    try {
      ...
      if (!firstDoc || !firstDoc.metadata || !firstDoc.metadata.fileName) {
        this.logger.error('Cannot determine cache file path from document metadata');
        return false;
      }
      ...
      const serializableData = {
        ...data,
        documentTrees: data.documentTrees ? this.serializeDocumentTrees(data.documentTrees) : undefined,
      };

      // JSON으로 변환하여 저장
      await fs.promises.writeFile(cacheFilePath, JSON.stringify(serializableData, null, 2));
  ...
  private async backupInvalidCache(cacheFilePath: string): Promise<void> {
    try {
      ...
      this.logger.info(`Invalid cache backed up to ${backupPath} and original deleted`);
    } catch (error) {
  ...
  public async logCacheStatus(cacheFilePath: string): Promise<void> {
    try {
      ...
      this.logger.info(
        `Cache file: ${cacheFilePath}, size: ${fileSizeInMB.toFixed(2)} MB, last modified: ${lastModified}`,
      );
    } catch (error) {
  ...
  /**
   * 벡터 캐시 파일만 찾기 (workspace 설정 파일 제외)
   */
  public async findCacheFiles(): Promise<string[]> {
    try {
      ...
      this.logger.info(`Found ${cacheFiles.length} vector cache files (excluding workspace settings)`);
      return cacheFiles;
    } catch (error) {
  ...
  public async invalidateCacheForRepository(owner: string, repo: string): Promise<void> {
    try {
      ...
      if (fs.existsSync(cacheFilePath)) {
        ...
        this.logger.info(`No cache found for repository: ${owner}/${repo}`);
      }
    } catch (error) {
      this.logger.error(`Error invalidating cache for repository ${owner}/${repo}:`, error);
    }
  }
  ...
  public async loadEmbeddingsCache(
    ...
    try {
      ...
      // 캐시 파일 읽기
      const cacheData = JSON.parse(await fs.promises.readFile(cacheFilePath, 'utf-8'));
      ...
      if (cacheData.documentTrees) {
        cacheData.documentTrees = this.deserializeDocumentTrees(cacheData.documentTrees);
        this.logger.info(`Loaded document trees for ${Object.keys(cacheData.documentTrees).length} files`);
      }

      this.logger.info(`Successfully loaded embeddings cache with ${cacheData.documents.length} documents`);
   ...
   */
  private serializeDocumentTrees(trees: Map<string, DocumentTree>): Record<string, any> {
   ...
   */
  private deserializeDocumentTrees(serialized: Record<string, any>): Map<string, DocumentTree> {
    ...
    Object.entries(serialized).forEach(([fileName, data]) => {
      ...
      const sectionMap = new Map(data.sectionMapEntries) as Map<string, ExtendedNode>;
