...
export class DocumentEnhancer {
  ...
  public async enhanceDocuments(documents: Document<DocumentMetadata>[]): Promise<Document<DocumentMetadata>[]> {
    ...
    console.info(
      `Pre-scan: ${uniqueUrls.length} unique URLs found, ${totalCachedUrls} already cached (${Math.round((totalCachedUrls / uniqueUrls.length) * 100)}% cache hit rate)`,
    );
    ...
    console.info(
      `Enhanced documents: ${enhancedDocuments.length} total (${webContentCount} web contents added, ${cachedWebContentCount} cached web contents reused)`,
    );
