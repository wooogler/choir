...
export class ConversationCache {
  ...
  private cleanupExpiredEntries(): void {
    ...
    if (cleanedCount > 0) {
      Logger.info(`ConversationCache: Cleaned up ${cleanedCount} expired entries. Current cache size: ${this.cache.size}`);
    }
  }
  ...
  async getOrFetchHistory(
    ...
    if (cached && (now - cached.timestamp) < cached.ttl) {
      Logger.info(`ConversationCache: Cache HIT for channel ${event.channel}${event.thread_ts ? ` thread ${event.thread_ts}` : ''}`);
      return cached.messages;
    }

    // Cache miss - fetch from API
    Logger.info(`ConversationCache: Cache MISS for channel ${event.channel}${event.thread_ts ? ` thread ${event.thread_ts}` : ''} - fetching from API`);
    ...
    try {
      ...
      Logger.info(`ConversationCache: Cached ${messages.length} messages for channel ${event.channel}. Cache size: ${this.cache.size}`);
      return messages;
    } catch (error) {
  ...
  addMessageToCache(
    ...
    for (const [key, entry] of this.cache.entries()) {
      ...
      const shouldUpdate = key.startsWith(`${channelId}_`) && 
        (threadTs ? key.includes(`_${threadTs}_`) : key.includes('_main_'));
      ...
      if (shouldUpdate) {
        ...
        const maxResults = keyParts.length >= 5 ? parseInt(keyParts[4]) : 5;
    ...
    if (updatedCount > 0) {
      Logger.info(`ConversationCache: Added message to ${updatedCount} cache entries for channel ${channelId}${threadTs ? ` thread ${threadTs}` : ''}`);
    }
  }
  ...
  invalidateChannel(channelId: string, threadTs?: string): void {
    ...
    for (const [key] of this.cache.entries()) {
      ...
      const shouldInvalidate = key.startsWith(`${channelId}_`) || 
        (threadTs && key.includes(`_${threadTs}_`));
    ...
    if (invalidatedCount > 0) {
      Logger.info(`ConversationCache: Invalidated ${invalidatedCount} cache entries for channel ${channelId}${threadTs ? ` thread ${threadTs}` : ''}`);
    }
  }
