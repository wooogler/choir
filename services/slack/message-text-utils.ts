...
function formatDiffSummary(diffContent: any): string {
  ...
  if (typeof diffContent === 'object' && diffContent.type === 'rich_text') {
    ...
    if (removed.length > 0) {
      ...
      const truncatedRemoved = removedContent.length > 150 
        ? removedContent.substring(0, 150) + '...' 
        : removedContent;
    ...
    if (added.length > 0) {
      ...
      const truncatedAdded = addedContent.length > 150 
        ? addedContent.substring(0, 150) + '...' 
        : addedContent;
 ...
 */
export function createComprehensiveText(
  baseText: string, 
  blocks: any[], 
